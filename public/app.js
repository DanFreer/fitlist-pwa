...
const USERS = [{name:'Parent 1',pin:'1234'},{name:'Partner',pin:'5678'}];
...
// ── Persistence ───────────────────────────────────────────────────────────────
function save() { try { localStorage.setItem('fitlist-v3', JSON.stringify(kids)); } catch(e){} }
function load() {
  try {
  ...
  kids = [
    { id:'demo-emma', name:'Emma', age:'8', emoji:'🦋', color:'#A29BFE', style:'Loves pastels & floral, prefers leggings over jeans, no scratchy tags',
      ...
      buys:[{text:'Pink rain jacket - Target',date:'2025-10-01'},{text:'3-pack leggings - H&M',date:'2025-09-15'}]
    },
    { id:'demo-jake', name:'Jake', age:'11', emoji:'🦖', color:'#FF6B6B', style:'Dinosaurs & gaming graphics, athletic fit only, hates jeans',
      ...
      buys:[{text:'3-pack graphic tees - Amazon',date:'2025-10-10'}]
    },
  ];
}
...
function renderUserBtns() {
  ...
  el.innerHTML = USERS.map(u=>`
    <button class="topbtn${currentUser.name===u.name?' active':''}" onclick="switchUser('${u.name}')">${u.name}</button>
  `).join('');
}
...
function switchUser(name) {
  ...
  pinTarget = u;
  document.getElementById('pin-modal-sub').textContent = `Enter PIN for ${u.name}`;
...
function renderKidHeader(k) {
  ...
  document.getElementById('kid-avatar').style.setProperty('--kid-color', k.color);
  if (k.photo) {
      ...
      ${!claimed?`
        <div class="claim-row" id="claim-row-${g.id}">
          ...
          <div class="claim-form" id="claim-form-${g.id}" style="display:none">
            <input class="claim-input" id="claim-input-${g.id}" type="text" value="${currentUser.name}" placeholder="Your name…" onkeydown="if(event.key==='Enter')submitClaim('${g.id}')"/>
...
function addBuy() {
  ...
  const text=input.value.trim(); if(!text)return;
  const k=activeKid(); if(!k)return;
  k.buys.unshift({text,date:new Date().toISOString().slice(0,10)});
...
function showFamilyClaimForm(gid,kid) {
